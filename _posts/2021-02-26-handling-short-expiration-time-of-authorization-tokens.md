...
![](../assets/img/Pasted image 20210226084418.png)
...
![Request to the endpoint responsible for generating a new access token for the API](../assets/img/Pasted%20image%2020210226084602.png)

![Request to the endpoint that uses the access token](../assets/img/Pasted%20image%2020210226084638.png)
...
Moving on, once I realized that I had to deal with this short time frame before having to request a new token, I began to do what every good penetration tester does: [begging on my knees for google to bring me a solution](https://miro.medium.com/max/455/1*snTXFElFuQLSFDnvZKJ6IA.png). Jokes apart, I knew that Burp had something for handling sessions, but I had never used it before, nor I knew where those options were. Even so, It wasn’t dificult to find them:

![Location of “Session Handling Rules” and “Macros” inside Burp Suite](../assets/img/Pasted%20image%2020210226084732.png)
...
![Adding a new Session Handling Rule](../assets/img/Pasted%20image%2020210226084843.png)
...
![Scope definition for a new session rule](../assets/img/Pasted%20image%2020210226084954.png)
...
![First part of the action editor for a session handling rule](../assets/img/Pasted%20image%2020210226085033.png)
...
4.  What to look when inspecting the response. In my case I had to look for the literal string “401” in the response body as this was the bahaviour i got when the token expired. Note that we can also look for [regex](https://www.pinterest.com/pin/270708627573628298/) and make the search case (in)sensitive.
...
![Second part of the action editor for a session handling rule](../assets/img/Pasted%20image%2020210226085117.png)
...
So for this specific need, there’s this Burp extension called “[Custom Parameter Handler](https://github.com/PortSwigger/custom-parameter-handler)” or CPH. What it does is “modifying HTTP messages with surgical precision, even when using macros”. More precisely for my scenario, it allows, among other things, [search and replace inside requests and responses](https://github.com/elespike/burp-cph/wiki/03.-Find-and-replace).
...
![Recording macros in Burp Suite](../assets/img/Pasted%20image%2020210226085239.png)
...
![Recording a macro in Burp Suite](../assets/img/Pasted%20image%2020210226085316.png)
...
![Configuring a macro request to handle parameters and cookies](../assets/img/Pasted%20image%2020210226085352.png)
...
![CPH options](../assets/img/Pasted%20image%2020210226085431.png)
...
![Configuring a new CPH tab](../assets/img/Pasted%20image%2020210226085509.png)
...
![Extracting a string from a response and replacing a string of the original request with it](../assets/img/Pasted%20image%2020210226085534.png)

1.  Here we setup a regex for searching the string to be replaced in the original request. Notice that the regex starts with “ey”. It will match exactly the access token (a [JWT](https://jwt.io/)) that expired.
